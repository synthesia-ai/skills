...
| Frontend connects but the agent/avatar never joins | Agent worker not running, or the frontend points at a different LiveKit project. | Keep `python agent.py dev` running; use the same `LIVEKIT_URL` / API key / secret in the frontend and the agent. |
