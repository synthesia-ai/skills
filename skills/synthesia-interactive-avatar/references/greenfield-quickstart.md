...
> **Dispatch note:** the boilerplate sets `agent_name=` in `WorkerOptions`, which makes agent dispatch explicit. The Agent Console handles this for you when you select the agent from the dropdown, but a custom frontend (options 2–4) must dispatch the agent into the room — or remove `agent_name` to restore automatic dispatch.
